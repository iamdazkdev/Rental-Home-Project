...
// Lock duration in minutes (from config - 30 minutes)
const DEFAULT_LOCK_DURATION = BOOKING_INTENT_TIMEOUT_MINUTES;
...
const extendIntentLock = async (intentId, additionalMinutes = 5) => {
    try {
        ...
        // Max extension is 30 minutes total from original lock (from config)
        const maxExpiresAt = new Date(intent.lockedAt.getTime() + BOOKING_INTENT_TIMEOUT_MS);
