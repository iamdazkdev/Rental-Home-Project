...
class BookingService {
    ...
    async createBookingIntent(bookingData, paymentType) {
        try {
            ...
            // Create BookingIntent (expires based on config - 30 minutes)
            const expiresAt = new Date(Date.now() + BOOKING_INTENT_TIMEOUT_MS);
    ...
    async createBookingFromPayment(tempOrderId, transactionId, paymentData) {
        try {
            console.log('📝 createBookingFromPayment called with:', {
                tempOrderId,
                transactionId,
            ...
            console.log('🔍 BookingIntent lookup result:', {
                found: !!bookingIntent,
                intentId: bookingIntent?.intentId,
                status: bookingIntent?.status
            });

            if (!bookingIntent) {
                // Fallback: try old PendingBooking for backward compatibility
                console.log('⚠️ BookingIntent not found, trying PendingBooking...');
                const pendingBooking = await PendingBooking.findOne({tempOrderId});

                if (!pendingBooking) {
                    console.error('❌ Neither BookingIntent nor PendingBooking found for:', tempOrderId);
                    throw new Error('BookingIntent not found or expired');
                }

                console.log('✅ Using PendingBooking (old flow)');
            ...
            console.error('❌ Error creating booking from payment:', {
    ...
    async createBookingFromPendingBooking(pendingBooking, transactionId, paymentData) {
        try {
            ...
            await PaymentHistory.create({
                ...
                amount: paidAmount,
                method: 'vnpay',  // ✅ Correct field name
                type: pendingBooking.paymentMethod === 'vnpay_full' ? 'full' : 'deposit',  // ✅ Correct field name and enum
                transactionId,
                status: 'paid',  // ✅ Valid enum value
                notes: `Payment for booking ${booking._id}`,  // ✅ Use 'notes'
                paidAt: new Date()
            });
    ...
    async confirmCashPayment(bookingId, hostId, paymentDetails) {
        try {
            ...
            await PaymentHistory.create({
                ...
                type: booking.paymentType === 'deposit' ? 'remaining' : 'full',  // ✅ Correct field name and enum
                transactionId: null,
                status: 'paid',  // ✅ Valid enum value
                notes: notes || `Cash payment confirmed for booking ${booking._id}`,  // ✅ Use 'notes'
                paidAt: new Date()
            });
        ...
        try {
            ...
            if (booking.paymentMethod.includes('vnpay')) {
                ...
                await PaymentHistory.create({
                    ...
                    method: 'vnpay',  // ✅ Correct field name
                    type: 'refund',  // ✅ Correct field name (already correct enum)
                    transactionId: null,
                    status: 'refunded',  // ✅ Valid enum value
                    notes: `Refund for ${reason}`,  // ✅ Use 'notes'
                    refundedAt: new Date()
                });
            }
    ...
    async releasePaymentToHost(booking) {
        try {
            ...
            await PaymentHistory.create({
                ...
                amount: hostAmount,
                method: 'vnpay',  // ✅ Correct field name
                type: 'adjustment',  // ✅ Use 'adjustment' for host payout
                transactionId: null,
                status: 'paid',  // ✅ Valid enum value
                notes: `Payout to host (after 10% platform fee)`,  // ✅ Use 'notes'
                paidAt: new Date()
            });
