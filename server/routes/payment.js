...
router.get("/vnpay-return", async (req, res) => {
    try {
        ...
        if (!tempOrderId) {
            ...
            return res.redirect(
                `${process.env.CLIENT_URL || 'https://rental-home-iamdazk.surge.sh'}/payment/result?status=error&message=Missing order ID`
            );
        }
        ...
        if (!pendingBooking) {
            ...
            return res.redirect(
                `${process.env.CLIENT_URL || 'https://rental-home-iamdazk.surge.sh'}/payment/result?status=error&message=Booking data not found or expired`
            );
        }
        ...
        if (isPaymentSuccessful) {
            ...
            return res.redirect(
                `${process.env.CLIENT_URL || 'https://rental-home-iamdazk.surge.sh'}/payment/result?status=success&bookingId=${bookingId}&transactionNo=${transactionNo}&paymentStatus=${paymentStatus}`
            );
        } else {
            // Payment failed
            ...
            return res.redirect(
                `${process.env.CLIENT_URL || 'https://rental-home-iamdazk.surge.sh'}/payment/result?status=failed&message=Payment failed or cancelled`
            );
        }
        ...
        return res.redirect(
            `${process.env.CLIENT_URL || 'https://rental-home-iamdazk.surge.sh'}/payment/result?status=error&message=${encodeURIComponent(error.message)}`
        );
    }
});
