...
// Admin (Layout only, pages use Data API lazy)
...
const router = createBrowserRouter(
    createRoutesFromElements(
        <Route element={<RootLayout />} errorElement={<ErrorBoundary />}>
            {/* ========== PUBLIC ROUTES ========== */}
            <Route path="/" element={<AdminRedirect><Suspended fallback={ListingGridSkeleton}><HomePage/></Suspended></AdminRedirect>}/>
            ...
            <Route path="/admin" element={<AdminRoute><Suspended fallback={PageSkeleton}><AdminLayout/></Suspended></AdminRoute>}>
                <Route 
                    ...
                    lazy={async () => {
                        ...
                        return { Component: m.default, loader: m.adminDashboardLoader };
                    }}
                />
                <Route 
                    path="dashboard" 
                ...
                <Route 
                    path="users" 
                    lazy={async () => {
                        const m = await import("./pages/admin/UserList");
                        return { Component: m.default, loader: m.userListLoader, action: m.userListAction };
                    }}
                />
                <Route 
                    path="users/:id" 
                    lazy={async () => {
                        const m = await import("./pages/admin/UserDetail");
                        return { Component: m.default, loader: m.userDetailLoader, action: m.userDetailAction };
                    }}
                />
                <Route 
                    path="verifications" 
                    lazy={async () => {
                        const m = await import("./pages/admin/VerificationManagement");
                        return { Component: m.default, loader: m.verificationManagementLoader, action: m.verificationManagementAction };
                    }}
                />
            </Route>
            
            {/* Admin Management requires Outlet for nested routing context if needed, or simply render Component */}
            <Route path="/admin/manage" element={<AdminRoute><Outlet /></AdminRoute>}>
                <Route 
                    index
                    lazy={async () => {
                        const m = await import("./pages/admin/AdminManagement");
                        return { Component: m.default, loader: m.adminManagementLoader, action: m.adminManagementAction };
                    }}
                />
            </Route>
