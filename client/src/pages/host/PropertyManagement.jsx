    ...
    const fetchProperties = async () => {
        try {
            ...
            if (response.ok) {
                ...
                // Filter only "An entire place" listings (exclude Room(s))
                const entirePlaceListings = data.filter(p => {
                    const type = p.type?.toLowerCase() || '';
                    return type.includes('entire') || type === 'entire place';
                });

                console.log("🏡 Entire Place listings:", entirePlaceListings.length);
                console.log("📊 Total listings (before filter):", data.length);

                // Store raw data (already filtered for entire place)
                setAllProperties(entirePlaceListings);

                // Debug: Check isActive values
                entirePlaceListings.forEach((p, i) => {
                ...
                let filteredData = entirePlaceListings;
                if (filter === "active") {
                    // Show active listings (isActive === true OR isActive is undefined/null)
                    filteredData = entirePlaceListings.filter(p => p.isActive !== false);
        ...
        <>
            ...
            <div className="property-management">
                <div className="management-header">
                    <h1>🏡 My Entire Place Listings</h1>
                    <p>Manage your entire place rentals (short-term bookings)</p>
                    <button className="create-new-btn" onClick={() => navigate("/create-listing")}>
                        + Create New Entire Place
                    </button>
                </div>
