    ...
    const fetchApplications = async () => {
        try {
            ...
            const response = await axios.get(
                `${API_BASE_URL}/room-rental/applications/tenant/${user._id}`
            );
                            ...
                            <div key={app._id} className="application-card">
                                ...
                                <div className="application-timeline">
                                    ...
                                    {app.interview?.scheduledDate && (
                                        <div
                                            className={`timeline-item ${app.status === 'interview_completed' || app.status === 'offer_made' || app.status === 'contracted' ? 'completed' : 'pending'}`}>
                                            <div className="timeline-icon">
                                                <Calendar size={16}/>
                                            </div>
                                            <div className="timeline-content">
                                                ...
                                                {app.interview.location &&
                                                    <p className="interview-location">📍 {app.interview.location}</p>}
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {app.offer && (
                                        <div
                                            className={`timeline-item ${app.status === 'offer_accepted' || app.status === 'contracted' ? 'completed' : 'pending'}`}>
                                            <div className="timeline-icon">
                                                <FileText size={16}/>
                                            </div>
                                            <div className="timeline-content">
